...
class CustomerApplications extends React.Component {
  ...
  fetchRebates(completed = false, name = '') {
    fetch(
      `${appUrl}/admin/rebate_forms?utf8=✓&completed=${completed}&name=${name ||
        ''}`,
      {
        method: 'GET',
        headers: {
      ...
      <Fragment>
        <div className="pure-u-1-2 rebate-search-box">
          <div className={'rebate-tabs flex-row'}>
            {map(
              [['Not Signed', 'not-completed'], ['Signed', 'completed']],
              ([key, value]) => (
                <button
                  key={key}
                  className={
                    this.state.applicationState === value
          ...
          {!(this.state.applicationState === 'completed') && (
            <Form onSubmit={this.onSubmit.bind(this)}>
              {({ handleSubmit }) => (
                <form className="pure-form" onSubmit={handleSubmit}>
                  ...
                  <button
                    className="pure-button pure-button-primary rebate-search-button"
                    type="submit"
                  >
                    Search
                  </button>
        ...
        {this.state.rebateForms && this.state.rebateForms[0] && (
            ...
            <table className="pure-table pure-table-bordered rebate-results-table">
              ...
              <tbody className="rebate-results-table-body">
                {map(this.state.rebateForms, (rebateForm, key) => {
                  ...
                  return (
                    <tr
                      key={`${key}-${fields.full_name}`}
                      className="rebate_form.completed"
                    >
                      ...
                      {property ? (
                        <td className="rebate-results-table-cell">
                          {property.location} <br /> {property.suburb} <br />{' '}
                          {property.town_city}
                        </td>
                      ) : (
                        <td className="rebate-results-table-cell" />
