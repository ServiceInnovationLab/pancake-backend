...
class EditRebateForm extends React.Component {
  constructor(props) {
    ...
    const applicantKeys =
      otherIncome && otherIncome.applicant
        ? Object.keys(otherIncome.applicant)
        : [];
    const partnerKeys =
      otherIncome && otherIncome.partner
        ? Object.keys(otherIncome.partner)
        : [];
  ...
  addNewIncomeValue(values) {
    this.setState({
      otherIncomeFields: this.state.otherIncomeFields.concat(
        values.newIncomeField,
      ),
    });
  ...
  onSubmit(values) {
    ...
    return values.newIncomeField
      ...
      : fetch(`${appUrl}/admin/rebate_forms/${this.props.rebateForm.id}`, {
          ...
          console.log('res', res);
          if (res.ok)
            window.location = `${appUrl}/admin/rebate_forms/${
              this.props.rebateForm.id
            }`;
  ...
  render() {
    const { rebateForm, property, ratesBills, isReadOnly } = this.props;
    ...
    return (
      ...
      >
        {({ handleSubmit, values }) => {
          ...
          return (
            <form className="rebate-edit-form" onSubmit={handleSubmit}>
              <div className="flex-row">
                {map(customerDetailFields, (field, index) => {
                  if (indexOf([1, 4, 6, 8, 10, 12], index) >= 0)
                    field.withMargin = true;
              ...
              {!isReadOnly && (
                <Fragment>
                  ...
                  <div className="rebate-submit-button-wrapper">
                    <button
                      className="one-third rebate-add-income-button rebate-search-button"
                      type="submit"
                    >
                      Submit
                    </button>
